...
interface ModalListaEsperaSemPreferenciaProps {
  aberto: boolean
  onFechar: () => void
  tenantId: string
  barbeiroId: string
...
export function ModalListaEsperaSemPreferencia({
  aberto,
  onFechar,
  tenantId,
  barbeiroId,
  ...
  cores,
  nomeEstabelecimento
}: ModalListaEsperaSemPreferenciaProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Data fictícia para expiração (7 dias a partir de hoje)
      // Usamos uma data futura genérica já que não há data específica
      const dataExpiracao = addDays(new Date(), 7)
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          data: format(new Date(), 'yyyy-MM-dd'), // Data de hoje como referência
          horario_desejado: null, // NULL indica "qualquer horário"
      ...
      if (error) {
        if (error.code === '23505') {
          setMensagemErro('Você já está na lista de espera para este profissional')
          setStatus('erro')
          return
        }
        throw error
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <Bell className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: cores.secundaria }} />
                          <div>
                            <p className="text-sm font-medium" style={{ color: cores.secundaria }}>
                              Sem preferência de horário
                            </p>
                            <p className="text-xs mt-1" style={{ color: cores.destaque }}>
                              Você será notificado quando <strong>qualquer horário</strong> com {barbeiroNome} for liberado.
                            </p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          <Clock className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: cores.secundaria }} />
                          <div>
                            <p className="text-xs" style={{ color: cores.destaque }}>
                              Ideal para quem tem flexibilidade de horário e quer garantir o primeiro horário que surgir.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <p className="text-xs text-center" style={{ color: cores.destaque }}>
                        Válido por 7 dias. Você receberá uma mensagem no WhatsApp assim que qualquer horário for liberado.
                      </p>
                      ...
                      <button
                        ...
                        className="w-full py-3.5 rounded-xl font-semibold text-sm transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        style={{ 
                  ...
                  {status === 'sucesso' && (
                    ...
                    >
                      ...
                      <p className="text-xs" style={{ color: cores.destaque }}>
                        Avisaremos por WhatsApp quando qualquer horário liberar com {barbeiroNome}.
                      </p>
                    </motion.div>
                  )}
...
export default ModalListaEsperaSemPreferencia
